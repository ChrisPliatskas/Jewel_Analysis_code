...
TFile fout(outname,"RECREATE");
 static const int nR = 5; // radii 0.1, 0.2, 0.3, 0.4

  TH2F*hJetAreaPt[iR]={0};
  TH1F *hJetPt[nR] = {0};
  TH1F *hJetEta[nR] = {0};
...
for (Int_t iR=0; iR<nR; iR++){
	Float_t Rjet = (iR+1)*Rstep;
        ...
        histname = TString::Format("hJetPtArea_R%02d",int(Rjet*10));
        htitle = TString::Format("Jet area for R=%.1f;p_{t} (GeV/c);A_{jet,R=%.1f}",Rjet,Rjet);
        hJetAreaPt[iR] = new TH2F(histname,htitle,NPtBins,PtBin_min,PtBin_max,100,0,1);
...
//Addition of Thermal particles here
...
*/
		...
		hJetAreaPt[iR]->Fill(AcceptedJets[iR].at(j).area(),evt->weights()[0]);
                if(iR==nR-2){
			hJetPtMatched[iR+1]->Fill(Pt_high,evt->weights()[0]);
			hJetEtaMatched[iR+1]->Fill(AcceptedJets[iR+1].at(match_index).eta(),evt->weights()[0]);	
			hJetAreaPt[iR+1]->Fill(AcceptedJets[iR+1].at(match_index).area(),evt->weights()[0]);
                        	}
